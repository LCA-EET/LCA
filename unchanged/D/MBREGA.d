...
APPEND_EARLY HABREGA
	...
	THEN BEGIN M39
		...
		DO ~
			...
			SetGlobal("LCA_LetterSent", "GLOBAL",1)
		~
		GOTO M34
	...
	THEN BEGIN M40
		...
		DO ~
			...
			SetGlobal("LCA_LetterSent", "GLOBAL", 2)
		~
		GOTO M34
	...
	THEN BEGIN M41
		...
		*/
		DO ~
			...
			SetGlobal("LCA_LetterSent", "GLOBAL", 3)
		~
		GOTO M34
...
APPEND HABREGA
	...
	IF ~~ THEN BEGIN M30
		...
		IF ~
			Global("LCA_CorwinContinue", "GLOBAL", 2) // continues romance
		...
		IF ~
			Global("LCA_CorwinContinue", "GLOBAL", 1) // friendship
			!Alignment(Player1,MASK_EVIL)
		~
	...
	IF ~~ THEN BEGIN M34
		SAY @9  /* ~(You hand over the dagger and the letter to Brega) I'll have my courier pick these up first thing in the morning. It will take a few days for it to reach Baldur's Gate. Thank you again for your service, citizen.~ */
		IF ~~ THEN
		DO ~
			...
			DestroyItem("LCORW2")
			DestroyItem("LDUNC")
			SetGlobalTimer("LCA_STtoBG","GLOBAL",THREE_DAYS)
	...
	IF ~~ THEN BEGIN M38
		...
		DO ~
			SetGlobal("LCA_HelpBrega", "GLOBAL", 1)
	...
	IF ~~ THEN BEGIN M43
		...
		DO ~
			SetGlobal("LCA_DaggerCHROverride", "GLOBAL", 1)
		~
		GOTO M44
	END
