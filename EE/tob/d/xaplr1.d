APPEND ~PLAYER1~
IF ~
		Global("XA_LC_DaggerDestruction", "GLOBAL", 1)
	~ THEN BEGIN XA_DaggerDestruction
		...
		DO ~
			...
			SetGlobal("XA_LC_DaggerDestruction", "GLOBAL", 2)
			SetGlobal("XA_LC_Lodob", "GLOBAL", 1)
			StartCutSceneMode()
		...
		DO ~
			...
			SetGlobal("XA_LC_DaggerDestruction", "GLOBAL", 2)
			SetGlobal("XA_LC_BenoSpirit", "GLOBAL", 1)
			SetGlobal("XA_LC_Lodob", "GLOBAL", 1)
			StartCutSceneMode()
			StartCutScene("XADAG2A")
		~
	...
	~ THEN BEGIN XA_KanaDream
		...
		IF ~~ THEN
		DO ~
			SetGlobal("XA_LC_KanaDreamTimerSet", "GLOBAL",2)
			SetGlobalTimer("XA_LC_KanaDreamTimer", "GLOBAL", ONE_DAY)
			RestParty()
		~
		EXIT
	END
	...
	IF ~
		Global("XA_LC_TowerUnlock", "GLOBAL", 1)
	~ THEN BEGIN XA_TowerUnlocked
		...
		DO ~
			SetGlobal("XA_LC_TowerUnlock", "GLOBAL", 2)
			StartCutSceneMode()
	...
	IF ~
		Global("XA_LC_CaelarSuicide", "GLOBAL", 1)
	~ THEN BEGIN XA_CaelarSuicided
		...
		IF ~~ THEN
		DO ~
			SetGlobal("XA_LC_CaelarSuicide", "GLOBAL", 2)
