...
IF ~~ THEN BEGIN XA_StartDebrief01
	...
	IF ~~ THEN 
	DO ~
		SetGlobal("XA_LC_DukesDebriefed", "GLOBAL", 2)
		StartCutSceneMode()
...
~ THEN BEGIN XA_BenceWyrm_0
	...
	DO ~
		SetGlobal("XA_LC_BenceWyrm", "GLOBAL", 3)
...
IF ~
	Global("XA_LC_BenceDukeDebrief", "GLOBAL", 1)
	GlobalLT("XA_LC_ExtensionNeeded", "GLOBAL", 1)
~ THEN BEGIN XA_TimeToSeeTheDukes
	...
	DO ~
		SetGlobal("XA_LC_BenceDukeDebrief", "GLOBAL", 2)
	~
	GOTO XA_StartDukeDebriefing
	
	IF ~~ THEN REPLY @12 /* ~Can I have a little more time before we begin?~ */
	DO ~
		SetGlobalTimer("XA_LC_DukeDebriefOneHourTimer", "GLOBAL", ONE_HOUR)
		SetGlobal("XA_LC_ExtensionNeeded", "GLOBAL", 1)
	~
	GOTO XA_OneMoreHour
END
...
~ THEN BEGIN XA_BenceWyrm
	...
	DO ~
		SetGlobal("XA_LC_ExtensionNeeded", "GLOBAL", 2)
	~
	GOTO XA_StartDukeDebriefing	
END
